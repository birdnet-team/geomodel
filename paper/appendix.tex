...
\begin{table}[H]
\centering
\caption{GeoScore components, their weights, and the transformation applied to each raw metric. The weighting reflects the priorities of Section~\ref{sec:4-1-geoscore}: ranking quality and spatial equity dominate, while calibration and rare-species performance act as guards against degenerate solutions.}
...
C (env=.1, off) & 0.1 & off & 0.427 & 0.550 & 0.516 & 6.38 & 0.702 & 0.285 & 0.850 & 0.336 & 0.793 \\
C (env=.1, on) & 0.1 & on & 0.561 & 0.689 & 0.502 & 4.62 & 0.604 & 0.868 & 0.717 & 1.210 & 0.833 \\
C (env=.5, off) & 0.5 & off & 0.427 & 0.552 & 0.516 & 6.39 & 0.694 & 0.288 & 0.851 & 0.339 & 0.793 \\
...
\begin{table}[H]
\centering
\caption{Stage D, harmonics sweep (6 runs). Encoding resolution barely matters: GeoScore varies by 0.004 across all six spatial/weekly harmonic combinations, so the smallest setting is retained.}
\label{tab:stage-d}
\small
\resizebox{\linewidth}{!}{%
...
\textbf{E2 (jitter)} & \textbf{on} & \textbf{off} & \textbf{0.566} & \textbf{0.697} & \textbf{0.505} & \textbf{4.55} & \textbf{0.634} & \textbf{0.876} & \textbf{0.719} & \textbf{1.218} & \textbf{0.830} \\
E3 (yearly) & off & on & 0.565 & 0.695 & 0.504 & 4.61 & 0.632 & 0.874 & 0.719 & 1.215 & 0.827 \\
...
\begin{table}[H]
\centering
\caption{Stage F, observation cap sweep (3 runs plus baseline). Capping observations per cell is destructive: at cap 500 the model collapses entirely (mAP 0.002), and even cap 50{,}000 loses a third of the uncapped GeoScore. Frequency-based weighting handles the imbalance better than truncation.}
\label{tab:stage-f}
\small
\resizebox{\linewidth}{!}{%
...
\begin{table}[H]
\centering
\caption{Stage G, species vocabulary scaling (4 runs plus baseline). Absolute scores fall as the vocabulary grows, which is expected --- the task gets harder --- but the density ratio improves, indicating the gains are not concentrated in well-surveyed regions.}
\label{tab:stage-g}
\small
\resizebox{\linewidth}{!}{%
\begin{tabular}{lrrrrrrrrrr}
...
The optimization proceeded in two phases. An initial exploratory search (40 trials) used a centroid-based per-species range check: each candidate cell was tested against the distance to the species' geographic centroid. This identified the importance of tight ecological constraints but was found to penalize migratory species whose breeding and wintering grounds lie far from any centroid. A refined search (20 trials) corrected the distance metric to measure proximity to the nearest original observation of the species, providing a more ecologically appropriate criterion.

\textbf{Phase 1 --- Exploratory search (40 trials).} Top 10 trials by GeoScore:

\begin{table}[H]
\centering
\caption{Stage H, exploratory propagation search: the ten best of 40 TPE trials. The sparsity threshold and range cap dominate; $k$ and the environmental distance gate are comparatively free parameters.}
\label{tab:stage-h1}
\small
...
\begin{table}[H]
\centering
\caption{Stage H, refined propagation search with the corrected distance metric (20 trials). The top three configurations converge on a range cap near 1{,}100\,km and a sparsity threshold of 15--16, which is the configuration used in production.}
\label{tab:stage-h2}
\small
\begin{tabular}{rrrrrrrr}
\toprule
...
The watchlist comprises 18 endemic or restricted-range species selected to probe model behavior on geographically constrained distributions. Mean AP is computed per species and averaged across the watchlist to produce the Watchlist AP metric.

\begin{table}[H]
\centering
\caption{The 18 watchlist species: endemic and restricted-range taxa from three island archipelagos, plus a small set of otherwise restricted-range or critically endangered taxa. Mean AP across this set forms the watchlist component of GeoScore.}
\label{tab:watchlist}
\small
\begin{tabular}{lll}
\toprule
...
\midrule
hawgoo & Hawaiian Goose & Hawaii \\
hawhaw & Hawaiian Hawk & Hawaii \\
elepai & Hawaii Elepaio & Hawaii \\
apapan & Apapane & Hawaii \\
iiwi & Iiwi & Hawaii \\
hawama & Hawaii Amakihi & Hawaii \\
kea1 & Kea & New Zealand \\
nibkiw1 & North Island Brown Kiwi & New Zealand \\
takahe3 & South Island Takahe & New Zealand \\
riflem1 & Rifleman & New Zealand \\
tui1 & Tui & New Zealand \\
kokako3 & North Island Kokako & New Zealand \\
galhaw1 & Galápagos Hawk & Galápagos \\
galrai1 & Galápagos Rail & Galápagos \\
galpet & Galápagos Petrel & Galápagos \\
kagu1 & Kagu & New Caledonia \\
calcon & California Condor & California \\
whocra & Whooping Crane & North America \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Hyperparameter search space for the Bayesian optimization framework. Stage H tunes only the six propagation parameters; the remaining entries are available for general hyperparameter optimization.}
...
%% TODO: this appendix is empty in the source draft (format-specific tolerances and
%% validation methodology were never written). Commented out so the PDF does not show
%% a bare heading; restore the two lines below once the content exists.
%\section{Export Validation Tolerances}
%\label{sec:E-export-validation-tolerances}

\section{Post-Filter Study: Selection and Sampling}
\label{sec:F-post-filter-study-selection-and-sampling}

Supporting detail for Section~\ref{sec:7-post-filter-study-on-birdnet-app-submiss}
...
Europe still leads because H3 cells are equal-area units and Europe simply occupies more of them than Oceania does, but its share falls by a factor of 2.1 while the four under-represented regions gain five- to thirteen-fold.
...
\begin{table}[H]
\centering
\caption{Rejection rate by region at $t = 0.15$ (see Section~\ref{sec:7-4-caveats}).}
\label{tab:f-3}
\small
\begin{tabular}{lrrr}
\toprule
