...
BirdNET Geomodel extends that work with spatiotemporal species occurrence
predictions for post-filtering acoustic detections and supporting biodiversity
assessment.
...
- Respect privacy and local regulations, including when occurrence predictions
  are combined with recordings or precise location data.
- Use BirdNET Geomodel predictions responsibly, recognizing that model outputs
  are probabilistic and may contain false positives or false negatives.
...
- Account for accuracy tradeoffs introduced by model conversion and quantization.
...
- Combining BirdNET Geomodel with other systems for covert surveillance of people.
...
- Report the model version and conversion or quantization format used, so results remain reproducible.

## Branding

Forks, derivative applications, or redistributed versions should not use the
name "BirdNET", "BirdNET Geomodel", associated package names, or official
project branding in a manner that could reasonably imply endorsement by the
original project or its maintainers.
